...
public class NotificationService {
	...
	@Async
	public void sendNotification(JobsModel job, EmployerModel empData) {
		// TODO Auto-generated method stub

		List<CandidateModel> results = null;
		List<CandidateModel> fresherResult = null;
		List<CanLeadModel> leads = null;
		List<CanLeadModel> leadsExp = null;
		...
		if (job.getJobExp() == 0) {
			...
			/*
			 * fresherResult =
			 * candidateRepository.getFresherCandidates(job.getJobLocation()); leads =
			 * em.createQuery(
			 * "SELECT e FROM CanLeadModel e where e.candidateType = 'Fresher' and e.stopWACampaign = false and e.WACampaign = true"
			 * ) .getResultList(); crm.setFresherCount(fresherResult.size());
			 * crm.setLeadFresherCount(leads.size()); campaignReportRepository.save(crm);
			 * sendFreshersJobAlert(fresherResult, job, empData); sendLeadsJobAlert(leads,
			 * job, empData);
			 */
			
			if (job.getJobMaxExp() >0) {
				results = candidateRepository.getOneYearExperiencedCandidates(job.getIndustry(),
			...
			results = candidateRepository.getExperiencedCandidates(job.getIndustry(), job.getJobCategory(),
					job.getJobExp(), job.getJobMaxExp(), job.getJobLocation());

			leadsExp = em.createQuery(
					"SELECT e FROM CanLeadModel e where e.expYears >=:exp AND e.expYears <=:maxExp and e.candidateType = 'Experienced' and e.jobCategory = :jobCategory and e.industry = :industry and e.stopWACampaign = false and e.WACampaign = true")
					.setParameter("jobCategory", job.getJobCategory()).setParameter("industry", job.getIndustry())
					.setParameter("exp", job.getJobExp()).setParameter("maxExp", job.getJobMaxExp()).getResultList();
			crm.setMatchedCount(results.size());
			crm.setLeadExpCount(leadsExp.size());
			campaignReportRepository.save(crm);

			sendMatchedJobsAlert(results, job, empData);
			sendRelatedJobsAlert(groups, results, job, empData, industryId, crm);
			sendLeadsExpJobAlert(leadsExp, job, empData);
		}

	}

	private void sendLeadsExpJobAlert(List<CanLeadModel> leads, JobsModel job, EmployerModel empData) {
		// TODO Auto-generated method stub
		if (!leads.isEmpty()) {
			String jobLink = getJobLink(job.getId());

			for (CanLeadModel cm : leads) {
				...
				d.put("mn", "91" + String.valueOf(cm.getMobileNumber()));
				d.put("name", cm.getName());
	...
	private void sendLeadsJobAlert(List<CanLeadModel> leads, JobsModel job, EmployerModel empData) {
		// TODO Auto-generated method stub
		if (!leads.isEmpty()) {
	...
	private void sendRelatedJobsAlert(List<CfgFullTimeGroup> groups, List<CandidateModel> matchedJobs, JobsModel job,
		...
		if (!groups.isEmpty()) {
			...
			for (CfgFullTimeGroup c : groups) {
				...
				result = candidateRepository.getExperiencedRelatedCandidates(indModel.get().getIndustry(),
